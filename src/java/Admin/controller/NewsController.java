    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (service.equals("updateNews")) {
            ...
            if (submit == null) {
                ...
                ArrayList<NewsGroup> groups = daoG.getJustNews();
        ...
        if (service.equals("searchNews")) {
            ...
            int newsg_id = 0;
            String title = request.getParameter("title");
            String newsgroup_id = request.getParameter("newsgroup_id");
            if (newsgroup_id.isEmpty()) {
                list = dao.searchNews(title);
            } else {
                newsg_id = Integer.parseInt(newsgroup_id);
                list = dao.searchNewsById(title, newsg_id);
            }
            ArrayList<NewsGroup> groups = daoG.getAllNewsGroup();
            request.setAttribute("listG", groups);
            if (list.isEmpty()) {
                ...
                if (newsg_id == 0) {
                    request.setAttribute("newsgroup_id", null);
                } else {
                    request.setAttribute("newsgroup_id", newsg_id);
                }
